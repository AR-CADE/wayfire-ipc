...
void ipc_server_t::send_event(const char *json_string, enum ipc_event_type event)
{
    ...
    for (int i = 0; i < (int)ipc_client_list.size(); i++)
    {
        ...
        if (client->subscribed_events == IPC_WF_EVENT_TYPE_NONE || (client->subscribed_events & event_mask(event)) == 0)
        {
            continue;
        }  
            ...
            (uint32_t)std::string(json_string).size()))
        {
            ...
            /* send_reply destroys client on error, which also removes it from
                * the list, so we need to process current index again */
            i--;
        }
    }
}
        ...
        const char *payload, uint32_t payload_length)
{
    ...
    {
        LOGE("Client write buffer too big (",client->write_buffer_size,"), disconnecting client");
        client_disconnect(client);
        return false;
    }

   char *new_buffer = (char *)realloc(client->write_buffer, client->write_buffer_size);
	if (!new_buffer) {
    ...
    LOGD("Added IPC reply of type ",payload_type," to client ",client->fd," queue: ",
         payload);
    return true;
}
   ...
   std::string buf(payload_length, '\0');
    ...
    switch (payload_type)
    {
        ...
        case IPC_GET_OUTPUTS:
        {
            ...
            for (wf::output_t *output : outputs)
            {
                if (output->handle->enabled)
                {
                    ...
                    output_json["focused"] = wf::get_core().get_active_output() == output;
        ...
        case IPC_SUBSCRIBE:
        {
            ...
            for (Json::Value::ArrayIndex i = 0; i < request.size(); i++)
            {
                if (request[i].isString())
                {
                    ...
                    {
                        client->subscribed_events |= event_mask(IPC_I3_EVENT_TYPE_WORKSPACE);
                    } 
                    else if (event == IPC_I3_EVENT_SHUTDOWN)
                    {
                        client->subscribed_events |= event_mask(IPC_I3_EVENT_TYPE_SHUTDOWN);
                    }
                    else if (event == IPC_I3_EVENT_WINDOW) {
                        client->subscribed_events |= event_mask(IPC_I3_EVENT_TYPE_WINDOW);
                    }
                    else if (event == IPC_I3_EVENT_BINDING) {
                        client->subscribed_events |= event_mask(IPC_I3_EVENT_TYPE_BINDING);
                    } 
                    ...
                    {
                        client->subscribed_events |= event_mask(IPC_SWAY_EVENT_TYPE_INPUT);
                    } 
                    else 
                    {
                        client->subscribed_events = IPC_WF_EVENT_TYPE_NONE;
            ...
            if (has_tick) 
            {
                ...
                msg["payload"] = "";
                std::string msg_string = ipc_json::json_to_string(msg);
                send_reply(client, (int)IPC_I3_EVENT_TYPE_TICK, msg_string.c_str(), msg_string.size());
            }
            break;
        }
        ...
        case IPC_UNSUBSCRIBE:
        {
            ...
            for (Json::Value::ArrayIndex i = 0; i < request.size(); i++)
            {
                if (request[i].isString())
                {
                    ...
                    {
                        client->subscribed_events = client->subscribed_events & ~event_mask(event_type);
                    }
                }
            }
        ...
        case IPC_GET_SEATS:
        {
            ...
            Json::Value seat = ipc_json::describe_seat(wf::get_core().get_current_seat());
            seats.append(seat);
