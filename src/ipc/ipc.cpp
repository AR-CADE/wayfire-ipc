...
// Damn, another variant ... hoppefully we have our event_mask, in the worst case, i can implement a completly useless workaroud (but mandatory ;-) ) to fix it !!!!
...
class ipc_t : public wf::singleton_plugin_t<ipc_server_t>
{
  ...
  public:
    ...
    void fini() override
    {
        ...
        // // Set a timeout of 100 ms to give so time to all  client to handle the "exit" signal
        ...
        // fini_event_source =
        // wl_event_loop_add_timer(wf::get_core().ev_loop, handle_fini_timeout, nullptr);
  ...
  private:
    ...
    void signal_event(const enum ipc_event_type &signal, const std::string &json_string)
    {
    ...
    wf::signal_connection_t view_mapped = [=] (wf::signal_data_t *data)
    {
        ...
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_MAPPED, data, IPC_WF_EVENT_VIEW_MAPPED);
    };

    wf::signal_connection_t view_unmapped = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_I3_EVENT_TYPE_WINDOW, data, "close");
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_UNMAPPED, data, IPC_WF_EVENT_VIEW_UNMAPPED);
    };

    wf::signal_connection_t view_tiled = [=] (wf::signal_data_t *data)
    {
        //signal_window_event(IPC_I3_EVENT_TYPE_WINDOW, data, "move");
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_TILED, data, IPC_WF_EVENT_VIEW_TILED);
    };

    wf::signal_connection_t view_minimized = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_I3_EVENT_TYPE_WINDOW, data, "move");
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_MINIMIZED, data, IPC_WF_EVENT_VIEW_MINIMIZED);
    };
    ...
    wf::signal_connection_t view_fullscreened = [=] (wf::signal_data_t *data)
    {
        ...
        if (view->fullscreen != signal->state)
        {
            ...
            //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_FULLSCREENED, data, IPC_WF_EVENT_VIEW_FULLSCREENED);
        }       
    };

    wf::signal_connection_t view_layer_attached = [=] (wf::signal_data_t *data)
    {        
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_LAYER_ATTACHED, data, IPC_WF_EVENT_VIEW_LAYER_ATTACHED);
    };

    wf::signal_connection_t view_layer_detached = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_LAYER_DETACHED, data, IPC_WF_EVENT_VIEW_LAYER_DETACHED);
    };

    wf::signal_connection_t view_focused = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_I3_EVENT_TYPE_WINDOW, data, "focus");
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_FOCUSED, data, IPC_WF_EVENT_VIEW_FOCUSED);
    };

    wf::signal_connection_t view_change_workspace = [=] (wf::signal_data_t *data)
    {        
        signal_window_event(IPC_I3_EVENT_TYPE_WINDOW, data, "move");
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_CHANGE_WORKSPACE, data, IPC_WF_EVENT_VIEW_CHANGE_WORKSPACE);
    };

    wf::signal_connection_t view_pre_moved_to_output = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_PRE_MOVED_TO_OUTPUT, data, IPC_WF_EVENT_VIEW_PRE_MOVED_TO_OUTPUT);
    };

    wf::signal_connection_t view_attached = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_ATTACHED, data, IPC_WF_EVENT_VIEW_ATTACHED);
    };

    wf::signal_connection_t view_detached = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_DETACHED, data, IPC_WF_EVENT_VIEW_DETACHED);
    };

    wf::signal_connection_t view_set_sticky = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_SET_STICKY, data, IPC_WF_EVENT_VIEW_SET_STICKY);
    };
    ...
    {
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_APP_ID_CHANGED, data, IPC_WF_EVENT_VIEW_APP_ID_CHANGED);
    };
    ...
    wf::signal_connection_t view_title_changed = [=] (wf::signal_data_t *data)
    {
        ...
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_TITLE_CHANGED, data, IPC_WF_EVENT_VIEW_TITLE_CHANGED);           
    };
    ...
    wf::signal_connection_t view_geometry_changed = [=] (wf::signal_data_t *data)
    {
       ...
       // signal_window_event(IPC_WF_EVENT_TYPE_VIEW_GEOMETRY_CHANGED, data, IPC_WF_EVENT_VIEW_GEOMETRY_CHANGED);
    };
    ...
    wf::signal_connection_t view_above_changed = [=] (wf::signal_data_t *data)
    {
        ...
        signal_window_event(IPC_WF_EVENT_TYPE_VIEW_ABOVE_CHANGED, data, IPC_WF_EVENT_VIEW_ABOVE_CHANGED);
    };

    wf::signal_connection_t view_hints_changed = [=] (wf::signal_data_t *data)
    {
        signal_window_event(IPC_I3_EVENT_TYPE_WINDOW, data, "urgent");
        //signal_window_event(IPC_WF_EVENT_TYPE_VIEW_HINTS_CHANGED, data, IPC_WF_EVENT_VIEW_HINTS_CHANGED); 
    };
    ...
    wf::signal_connection_t output_added = [=] (wf::signal_data_t *data)
    {
        ...
        //signal_output_event(IPC_WF_EVENT_TYPE_OUTPUT_ADDED, data, IPC_WF_EVENT_OUTPUT_ADDED);
    };
    ...
    wf::signal_connection_t output_removed = [=] (wf::signal_data_t *data)
    {
        ...
        //signal_output_event(IPC_WF_EVENT_TYPE_OUTPUT_REMOVED, data, IPC_WF_EVENT_OUTPUT_REMOVED);
    };
        ...
        for (auto &dev : devices)
        {
            ...
            if (wlr_handle != nullptr)
            {
               if (wlr_handle->product == 1 && wlr_handle->vendor == 1)
               {
                    ...
                    data.device = dev;
                    signal_input_event(IPC_SWAY_EVENT_TYPE_INPUT, &data, "xkb_layout");
                    signal_input_event(IPC_SWAY_EVENT_TYPE_INPUT, &data, "xkb_keymap");
               }
            }
    ...
    Json::Value workspace_json_data(wf::signal_data_t *data, const std::string &change)
    {
        if (data == nullptr)
        {
            return Json::nullValue;
        ...
        if (change == "focus")
        {
            ...
            //signal.output = static_cast<wf::workspace_changed_signal*>(data)->output;
            //signal.new_viewport = static_cast<wf::workspace_changed_signal*>(data)->new_viewport;
            //signal.old_viewport = static_cast<wf::workspace_changed_signal*>(data)->old_viewport;
            //signal.carried_out = static_cast<wf::workspace_changed_signal*>(data)->carried_out;
        }
            ...
            if (signal.output == output)
            {
                ...
                for (int x = 0; x < wsize.width; x++)
                {
                    for (int y = 0; y < wsize.height; y++)
                    {
                        ...
                        {
                            Json::Value workspace = ipc_json::describe_workspace(wf::point_t{x,y}, output);
    ...
    std::string binding_json_data(wf::signal_data_t *data, const std::string &change = "run")
    {
        if (data == nullptr)
        {
            return nullptr;
...
/*     wf::signal_connection_t output_configuration_changed{
    ...
    void bind_output_events(wf::output_t *output)
    {
        ...
        output->connect_signal(IPC_WF_EVENT_VIEW_GEOMETRY_CHANGED, &view_geometry_changed);
        ...
        output->connect_signal("wm-actions-above-changed", &wm_actions_above_changed);
        output->connect_signal(IPC_WF_EVENT_VIEW_CHANGE_WORKSPACE, &view_change_workspace);

        //output->connect_signal(IPC_WF_EVENT_VIEW_LAYER_ATTACHED, &view_layer_attached);
        //output->connect_signal(IPC_WF_EVENT_VIEW_LAYER_DETACHED, &view_layer_detached);
        ...
        //output->connect_signal(IPC_WF_EVENT_OUTPUT_CONFIGURATION_CHANGED, &output_configuration_changed);
    ...
    void bind_view_events(wayfire_view view)
    {
        ...
        view->connect_signal(IPC_WF_EVENT_VIEW_GEOMETRY_CHANGED, &view_geometry_changed);

        //view->connect_signal("fullscreen", &view_fullscreened);
        //view->connect_signal(IPC_WF_EVENT_VIEW_APP_ID_CHANGED, &view_app_id_changed);
    ...
    void bind_core_events()
    {
        ...
        // wf::get_core().connect_signal(IPC_WF_EVENT_POINTER_BUTTON, &pointer_button);
