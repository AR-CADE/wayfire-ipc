...
bool criteria::matches_view(const Json::Value& view) 
{
	...
	if (this->urgent) {
        ...
        std::sort(urgent_views.begin(), urgent_views.end(), [](wayfire_view a, wayfire_view b)
        {
            	if (a->last_focus_timestamp < b->last_focus_timestamp) 
                {
                    return -1;
	...
	if (this->workspace) {
		...
		for (const wayfire_view& v: wf::get_core().get_all_views())
		{
			...
			{
				wf::point_t ws = wf::get_core().get_active_output()->workspace->get_view_main_workspace(v);
				Json::Value workspace = ipc_json::describe_workspace(ws, v->get_output());
		...
		case PATTERN_FOCUSED:
        {
            if (focused) {
                ...
                wf::point_t focused_ws = focused->get_output()->workspace->get_view_main_workspace(focused);
                Json::Value focused_workspace = ipc_json::describe_workspace(focused_ws, focused->get_output());
                Json::Value focused_workspace_name = focused_workspace.get("name", Json::nullValue);

                if (!focused_workspace_name.isNull() && focused_workspace_name.isString())
                {
                    std::string focused_workspace_name_str = focused_workspace_name.asString();
...
/* std::vector<struct criteria *> criteria_for_view(const wayfire_view& view, enum criteria_type types) {
...
void criteria::node_for_each_matched_container(Json::Value obj, Json::Value *containers) 
{
    if (obj.isNull())
    {
        return;
