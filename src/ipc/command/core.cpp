...
std::map<std::string, std::function<Json::Value(int argc, char **argv, command_handler_context *ctx)>> core_handler_map 
{ 
...
std::function<Json::Value (int argc, char **argv, command_handler_context *ctx)> core_handler(int argc, const char **argv) 
{
    if (!(argc > 0))
    {
        return nullptr;
