...
static RETURN_STATUS set_tranform(enum wl_output_transform transform, wf::output_t *output) 
{
    bool found = true;
...
Json::Value transform_handler(int argc, char **argv, command_handler_context *ctx)
{
	...
	if (!argc) {
        return ipc_json::build_status(RETURN_INVALID_PARAMETER, Json::nullValue, "Missing transform argument.");
	}
	...
	if (strcmp(*argv, "normal") == 0 ||
        ...
        return ipc_json::build_status(RETURN_INVALID_PARAMETER, Json::nullValue, "Invalid output transform.");
	}
    ...
    if (output == nullptr) {
        return ipc_json::build_status(RETURN_ABORTED, Json::nullValue, "Cannot apply relative transform to unknown output");
    ...
    if (argc > 1 &&
		...
		if (output_config->name != nullptr) {
            return ipc_json::build_status(RETURN_INVALID_PARAMETER, Json::nullValue, "Output config name not set");
		}
		if (strcmp(output_config->name, "*") == 0) {
            return ipc_json::build_status(RETURN_INVALID_PARAMETER, Json::nullValue, "Cannot apply relative transform to all outputs.");
		}
