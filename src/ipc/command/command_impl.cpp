...
Json::Value ipc_command::checkarg(int argc, const char *name, enum expected_args type, int val)
{
    ...
    if (!error_name.isNull())
    {
        std::string error = "Invalid " + std::string(name) +" command "
				"(expected "+ error_name.asString() +  std::to_string(val) +" argument" 
...
wayfire_view ipc_command::find_container(int con_id)
{
    ...
    auto view = std::find_if(views.begin(), views.end(), [&](const wayfire_view& v) { return ((uint32_t)con_id == v->get_id()) ;});
...
bool ipc_command::container_has_ancestor(wayfire_view descendant,
    ...
    if (std::any_of(chidlren.begin(), chidlren.end(), [&](const auto &child) { return child == ancestor; })) {
        return true;
    }
...
static void set_config_node(const Json::Value& node, bool node_overridden, command_handler_context *handler_context ) {
    ...
    if (type == "con" || type == "floating_con") {
		...
		for (const wayfire_view& view: wf::get_core().get_all_views())
		{
			...
			{
				wf::point_t ws = wf::get_core().get_active_output()->workspace->get_view_main_workspace(view);
				handler_context->workspace = ipc_json::describe_workspace(ws, view->get_output());
			}
		}
    } 
    ...
    do 
    {
        ...
        if (matched_delim == ';') 
        {
            ...
            if (*head == '[') 
            {
                ...
                if (!parsed) 
                {
                    res_list.append(ipc_json::build_status(RETURN_INVALID_PARAMETER, Json::nullValue, error));
                    free(error);
                    goto cleanup;
                }
		...
		if (!handler) {
            ...
            res_list.append(ipc_json::build_status(RETURN_SUCCESS, Json::nullValue, error.c_str()));
			ipc_tools::free_argv(argc, argv);
			goto cleanup;
		}
        ...
        {
            res_list.append(ipc_json::build_status(RETURN_ABORTED, Json::nullValue, "No matching node."));
        } 
        else 
        {
            ...
            res_list.append(!fail_res.isNull() ? fail_res : ipc_json::build_status(RETURN_SUCCESS));
        }
