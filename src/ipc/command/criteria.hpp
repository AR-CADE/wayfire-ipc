...
class criteria {
	public:
		...
		criteria(){};
		criteria(char *raw, char **error) {this->parse(raw, error);};
		~criteria() {this->destroy();};
		...
		*/
		// static std::vector<struct criteria *> for_view(const wayfire_view& view, enum criteria_type types);
	...
	#endif
		static void node_for_each_matched_container(Json::Value obj, Json::Value *containers);
