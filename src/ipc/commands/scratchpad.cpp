...
static void scratchpad_toggle_auto(wf::output_t * output) {
    ...
    Json::Value containers = ipc_json::get_i3_scratchpad_container_nodes_by_workspace(ws, output);
	...
	for (Json::ArrayIndex i = 0; i < containers.size(); ++i) {
		...
		if (!container.isNull()) {
			...
			if (focus != view) {
                LOGD("Focusing other scratchpad window (", view->get_title(), ") in this workspace"
                        );
    ...
    for (Json::ArrayIndex i = 0; i < containers.size(); ++i) {
		...
		if (!container.isNull()) {
			...
			if (focus != view) {
                LOGD("Focusing other scratchpad window (", view->get_title(), ") to this workspace"
                        );
...
Json::Value scratchpad_handler(int argc, char **argv, command_handler_context *ctx)
{
    ...
    if (strcmp(argv[0], "show") != 0) {
		return ipc_json::command_result(RETURN_INVALID_PARAMETER, "Expected 'scratchpad show'");
	}
